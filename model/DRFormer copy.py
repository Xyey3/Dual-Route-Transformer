...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.dim_proj = nn.Linear(configs.seg_num * configs.d_model, configs.d_model, bias=True)
        
        # self.route_fusion = RouteFusion(configs.d_model, dropout=configs.dropout)
        # # Prediction head
    ...
    #     # embedding
    #     # [B, L, V] -> [B, V, N, D]
    #     x = self.embedding(x_enc)          # [B, V, N, D]
    #     # Temporal path: global over segments
    #     time_in = x.mean(dim=2)             # [B, V, D]
    #     # Dimensional path: keep segment structure
    #     dim_in = x
        
    #     # Dual-route encoder
    #     # Output: [B, V, D], [B, V, N, D]
    #     (time_out, dim_out), attns = self.encoder(
    #         (time_in, dim_in),
    #         attn_mask=None
    #     )
    ...
    #     dim_out = self.dim_proj(dim_out) # [B, V, D]

    #     enc_out, _ = self.route_fusion(time_out, dim_out) 
