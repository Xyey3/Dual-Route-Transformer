...
class TemporalRoute(nn.Module):
    ...
    Input / Output: [B, L, var_num]
    """
    def __init__(self, var_num, dropout=0.1):
        super().__init__()
        # 此时 d_model 改为 var_num，Mamba在长度L上建模时间，通道数为var_num
        self.mamba = Mamba(
            d_model=var_num, 
            d_state=16, 
            d_conv=4, 
        ...
        self.norm = nn.LayerNorm(var_num)
...
class SCX_Block(nn.Module): 
    ...
    Input shape:  [B, V, L] (variables as tokens, seq_len as channels)
    Output shape: [B, K, L]
    """
    def __init__(self, var_num, heads, seq_len, attn_dropout, groups=10, return_full_attn=False):
        super().__init__()
        self.V = var_num          # number of variables
        self.K = groups           # number of clusters
        self.H = heads            # number of heads
        self.D = seq_len          # 此时通道维度为 seq_len (L)
        self.Dh = seq_len // heads

        # Tokens per cluster (P)
        self.P = max(math.ceil(var_num / groups), 1)

        # Learnable cluster centers: [1, K, L]
...
class DimensionalRoute(nn.Module):
    ...
    Input / Output: [B, L, var_num]
    """
    def __init__(self, factor, var_num, seq_len, n_heads, dropout=0.1):
        super().__init__()
        ...
        self.dim_router_sender = SCX_Block(
            var_num, n_heads, seq_len, dropout, groups=factor
        )

        self.dropout = nn.Dropout(dropout)
        self.norm_1 = nn.LayerNorm(seq_len)
        self.norm_2 = nn.LayerNorm(seq_len)
    ...
    def forward(self, x):
        # x: [B, L, var_num] 
        # 为了在变量维度建模，我们交换 L 和 var_num
        x_t = x.transpose(1, 2)  # [B, var_num, L]
        
        # SCX Routing
        route_tokens, route_select_attn = self.dim_router_sender(x_t)
        route_tokens = self.dropout(route_tokens)  # [B, K, L]
        
        # Global Route Distribution
        route_global = route_tokens.mean(dim=1, keepdim=True)  # [B, 1, L]
        dim_res = x_t + route_global # [B, var_num, L]

        # Gated refinement
        dim_norm = self.norm_1(dim_res)        # [B, var_num, L]
        gate = self.ffn(dim_norm)              # [B, var_num, L]
        dim_out = self.norm_2(dim_norm * gate) # [B, var_num, L]

        # 换回原形状 [B, L, var_num]
        return dim_out.transpose(1, 2), route_select_attn
...
class DualRouteInteractionBlock_(nn.Module):
    def __init__(self, factor, var_num, seq_len, n_heads, dropout=0.1):
        ...
        # 1. 时间维度的 Mamba 交互
        self.mamba_route = TemporalRoute(var_num=var_num, dropout=dropout)
